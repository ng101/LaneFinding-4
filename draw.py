...
def draw(img, binary_warped, left_fit, right_fit, roc, dist, ly, lx, ry, rx, 
        verbose=False):
    ...
    if verbose is True:
        ...
        color_warp[ly, lx] = [255, 0, 0]
        color_warp[ry, rx] = [255, 0, 0]

    # Warp the image to original image space using inverse perspective matrix (Minv)
    unwarped = topview.unwarp(color_warp)
    # Combine the result with the original image
    new_img = cv2.addWeighted(img, 1, unwarped, 0.3, 0)
